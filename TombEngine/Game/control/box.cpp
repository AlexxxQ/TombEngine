...
void TargetBox(LOTInfo* LOT, int boxNumber, ItemInfo* item)
{
	...
	else if (IsWaterZone(LOT))
	{
		LOT->Target.y = GetWaterCruiseY(item, LOT, *box);
	}
	else
	{
...
bool CanExpandToBox(LOTInfo* LOT, int fromBox, int toBox, int overlapFlags, int searchZone, const std::vector<int>& zone)
{
	...
	bool amphibiousSwimEdge = LOT->Zone == ZoneType::Amphibious && IsWetBox(from) && IsWetBox(to);
...
void CreatureMood(ItemInfo* item, AI_INFO* AI, bool isViolent)
{
	...
	auto targetOffset = (LOT->Zone == ZoneType::Flyer && IsEnemyOnLand(enemy)) ? Vector3i(0, GetFrame(*enemy).BoundingBox.Y1, 0) : Vector3i::Zero;
	...
	// Time-based unstuck for swimmers (see water_nav.cpp).
	TryUnstuckSwimmer(item, creature, LOT);
...
TARGET_TYPE CalculateTarget(Vector3i* target, ItemInfo* item, LOTInfo* LOT)
{
	...
	*target = item->Pose.Position;
	...
	// OG-faithful Y mode + LOS, shared by the swim navigator and the box-center override.
	bool ogYMode = ShouldUseOgWaterYMode(creature, item, LOT);
	bool losToEnemy = GetSwimNavLos(item, enemy, LOT);

	// Water-navigation override: clear-LOS direct homing at an in-water enemy, or
	// breadcrumb steering across vertical portals / water exits (see water_nav.cpp).
	if (TryGetSwimTarget(target, item, creature, enemy, LOT, ogYMode, losToEnemy))
