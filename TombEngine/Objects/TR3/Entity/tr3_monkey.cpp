...
namespace TEN::Entities::Creatures::TR3
{
	...
	constexpr auto MONKEY_VAULT_UP_SHIFT = CLICK(1.0f);
	constexpr auto MONKEY_VAULT_DOWN_SHIFT = CLICK(0.5f);
	constexpr auto MONKEY_VAULT_UP_PIVOT_OFFSET = BLOCK(0.25f);
	constexpr auto MONKEY_VAULT_DOWN_PIVOT_OFFSET = BLOCK(0.15f);
	...
	void MonkeyControl(short itemNumber)
	{
		...
		{
			switch (CreatureVault(itemNumber, angle, 2,
				MONKEY_VAULT_UP_SHIFT, MONKEY_VAULT_DOWN_SHIFT,
				MONKEY_VAULT_UP_PIVOT_OFFSET, MONKEY_VAULT_DOWN_PIVOT_OFFSET))
			{
			case 2:
